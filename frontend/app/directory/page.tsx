...
const DirectoryMap = dynamic(
  () => import("@/components/directory-map").then((m) => m.DirectoryMap),
  {
    ssr: false,
    loading: () => (
      <div className="w-full h-full bg-cloud animate-pulse rounded-[var(--r-xl)]" />
    ),
  },
);
...
export default function DirectoryPage() {
  ...
  useEffect(() => {
    if (searchParams.get("list") === "true" && user) {
    ...
    async function loadFirstPage() {
      try {
        ...
        try {
          ...
          const pageIndustries = firstPage.map((b) => b.industry);
          const pageRegions = firstPage.map((b) => b.region);

          setAvailableIndustries([
            ALL_INDUSTRIES,
            ...uniqueSorted([...options.industries, ...pageIndustries]),
          ]);
          setAvailableRegions([
            ALL_REGIONS,
            ...uniqueSorted([...options.regions, ...pageRegions]),
          ]);
        } catch {
          setAvailableIndustries([
            ALL_INDUSTRIES,
        ...
        setAvailableIndustries([
          ALL_INDUSTRIES,
          ...uniqueSorted(
            sampleBusinesses.map((business) => business.industry),
          ),
        ]);
        setAvailableRegions([
          ALL_REGIONS,
  ...
  return (
    <main>
      ...
      <div className="max-w-[1200px] mx-auto px-12 max-[960px]:px-6 py-20">
        ...
        <div className="flex items-center gap-3 mb-8 max-[640px]:flex-col max-[640px]:items-stretch">
          <SearchBar
          ...
          {userLocation && (
            <button
              onClick={() => setNearbySort((v) => !v)}
              className={cn(
                "btn-press focus-ring inline-flex items-center gap-1.5 font-sans text-[13px] font-medium px-4 py-2.5 rounded-full border transition-all duration-150 shrink-0",
                nearbySort
        ...
        {!isLoading && displayedBusinesses.length === 0 ? (
          ...
          <div
            className="flex gap-6 max-[960px]:flex-col"
            style={{ height: "70vh" }}
          >
            {/* Card list — left 1/3 */}
            <div className="w-1/3 max-[960px]:w-full max-[960px]:h-[40vh] overflow-y-auto space-y-4 pr-2 custom-scrollbar">
              {isLoading
                ? Array.from({ length: 4 }).map((_, i) => (
                    <div
                      key={i}
                      className="h-32 rounded-[var(--r-xl)] bg-cloud animate-pulse"
                    />
                  ))
                : displayedBusinesses.map((biz) => {
                    const dist =
                      nearbySort && userLocation && biz.lat != null && biz.lng != null
                        ? haversineKm(userLocation.lat, userLocation.lng, biz.lat, biz.lng)
                        : null;
                    return (
                      <div
                        key={biz.id}
                        ...
                        className={cn(
                          "transition-all duration-200 rounded-[var(--r-xl)] relative",
                          selectedBusinessId === biz.id &&
                            "ring-2 ring-[#3568B2] ring-offset-2",
                        )}
                      >
                        {dist !== null && (
                          <div className="absolute top-3 right-3 z-10 px-2 py-0.5 rounded-full text-[11px] font-semibold"
                            style={{ background: "rgba(27,107,79,0.1)", color: "#1B6B4F" }}>
                            {formatDist(dist)}
                          </div>
                        )}
                        <BusinessCard
                          ...
                          showSaveButton={!!(user && (user.role === "member" || user.role === "admin"))}
                          isSaved={savedBusinessIds.has(biz.id)}
                          isSaveLoading={savingBusinessIds.has(biz.id)}
