...
const actionBadge: Record<
  string,
  { variant: "positive" | "negative" | "caution" | "info"; label: string }
> = {
  ...
  suspended: { variant: "caution", label: "Suspended" },
  unsuspended: { variant: "positive", label: "Unsuspended" },
  deleted_member: { variant: "negative", label: "Deleted Member" },
}
      ...
      <Table>
        ...
        <TableBody>
          {actions.map((entry) => {
            ...
            const isMemberAction = !!entry.targetUsername

            return (
              <TableRow key={entry._id}>
                <TableCell className="font-medium">
                  {entry.targetUsername ? `@${entry.targetUsername}` : entry.businessName}
                </TableCell>
                <TableCell className="text-ink-400">
                  {isMemberAction ? "—" : entry.businessIndustry || "—"}
                </TableCell>
                <TableCell className="text-ink-400">
                  {isMemberAction
                    ? "—"
                    : [entry.businessCity, entry.businessRegion].filter(Boolean).join(", ") || "—"}
                </TableCell>
                <TableCell>
