...
export interface ApiBusiness {
  ...
  revenueRange?: "Pre-Revenue" | "<$1M" | "$1M-$5M" | "$5M-$10M" | "$10M+";
...
export function updateBusiness(id: string, data: Partial<CreateBusinessPayload>) {
...
export interface AdminActionEntry {
  _id: string;
  action: "approved" | "rejected" | "deleted" | "edited" | "suspended" | "unsuspended" | "deleted_member";
  businessName?: string;
...
export function suspendMember(userId: string) {
  return request<MemberUser>(`/admin/members/${userId}/suspend`, {
    method: "PATCH",
    auth: true,
  });
}
